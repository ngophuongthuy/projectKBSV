...
export default function OrderBookTable({ isChecked, onCheckboxChange }: OrderBookTableProps) {
  ...
  const [selectedOrder, setSelectedOrder] = React.useState<TableData | null>(null);

  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://64f7dd31824680fd217ee404.mockapi.io/api/Table');
        setTableData(response.data.map((item: TableData) => ({ ...item, isChecked: false })));
      } catch (error) {
  ...
  const handleCancelOrder = () => {
    if (selectedOrder) {
      const updatedData = tableData.filter(item => !(item.Ma === selectedOrder.Ma && item.Loailenh === selectedOrder.Loailenh));
      ...
      {tableData.map((item: TableData, index: number) => (
        <div
          ...
          className={`grid grid-cols-12 text-text_table_laber text-center items-center w-full text-xs relative  py-2
            ${index % 2 === 0 ? 'bg-bg_table border-y border border-bg_sidebar_table ': 'border-y border border-bg_sidebar_table ' }
        ...
        >
          ...
          <div className="grid col-span-4 grid-cols-4">
            ...
            <div className="col-span-1">{item.Thoigian.toString()}</div>
            ...
            <div className="col-span-4 relative flex justify-end items-center">
              {hovered === index && (
                <div className="absolute inset-0 flex justify-end items-center">
                  <button className="bg-color_yellow  text-text_buttom font-bold py-2 px-2 mb-4 rounded mr-2" onClick={handleOpenCancelDialog}>
                    <img src="/imageTable/trash.png" alt="" className="inline-block  mr-1" /> Hủy lệnh
                  </button>
                  <button className="bg-color_yellow  text-text_buttom font-bold py-2 px-2 mb-4 rounded">
                    <img src="/imageTable/edit-2.png" alt="" className="inline-block mr-1" /> Sửa lệnh
                  </button>
                </div>
              )}
            </div>
